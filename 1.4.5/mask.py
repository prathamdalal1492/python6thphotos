...
# Build an absolute filename from directory + filename
    filename = os.path.join(directory, '1.4.5_Images/Nagasaki_bomb.jpg')
# Read the image data into an array
    img = plt.imread(filename)
    height = len(img)   # line 15 
    width = len(img[0])     
    for row in range(100, 200):
        for column in range(100,200):
            print(img[row][column]) # red + green = yellow
...
def round_corners_of_all_images(directory=None):
    ...
    new_directory = os.path.join(directory, 'modified_2')
    try:
